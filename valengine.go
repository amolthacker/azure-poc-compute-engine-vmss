...
func main() {
	...
	logging.Info("Starting Veritas Valuation Engine [%s (QuantLib: %s)] ...", veVersion, qlVersion)

	k.HandleFunc("FwdRate", func(r *kite.Request) (interface{}, error) {
		return simulatedCompute("FwdRate", r)
	})

	k.HandleFunc("Pi", func(r *kite.Request) (interface{}, error) {
		return computePi(r)
	})
...
func computePi(r *kite.Request) (interface{}, error) {
	val := extractArgs(r)
...
func busyLoop(cmd string, r *kite.Request) (interface{}, error) {
	val := extractArgs(r)
	stressTime := strconv.Itoa(int(val * 0.5))
	args := []string{"-c", "2", "-t", stressTime}
