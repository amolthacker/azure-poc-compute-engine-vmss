...
func main() {
	...
	supportedMetrics := []string{"FwdRate", "NPV", "OptionPV", "IRDelta", "Pi"}
	metric := os.Args[3]
...
func usage(){
	logging.Info("go run eodservice.go <id(int)> <valengineDNS(string)> <metric(FwdRate|NPV|OptionPV|IRDelta|Pi)> <numRequests(int)>")
}
